...
The scanner interface was visually reviewed end to end on 2026-08-13. Idle, camera-starting, active-camera, successful-result, and recoverable-error states remain legible and contained across desktop, tablet, mobile, and short-landscape layouts.
...
- Viewfinder language: four continuous rounded corner brackets, four calibration ticks, a one-pixel scan beam, and a restrained off-region scrim
...
| Mobile          | 375 × 667  | Idle, starting, active, result, error | Compact brand treatment and stacked dialog actions remain legible           |
...
| Short landscape | 844 × 390  | Active                                | Viewfinder perimeter stays clear of brand, theme, stop, and camera controls |

All 25 full-state combinations plus the dedicated short-landscape active state had zero document-level horizontal and vertical overflow. Visible text actions remained on one line, and every interactive control measured at least 44 × 44 CSS pixels.
...
- non-overlapping active-camera overlays in portrait and short landscape;
- continuous rounded viewfinder bracket, mask, tick, border, and scan-line geometry;
